...
class UserManager(models.Manager):
    def register(self, form_data):
        ...
        if form_data['date_hired'] > datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d'):
        ...
        u = self.create(name=form_data['name'], username=form_data['username'], password=bcrypt.hashpw(form_data['password'].encode('utf-8'), bcrypt.gensalt()), date_hired=form_data['date_hired'])
        return (True, u)
